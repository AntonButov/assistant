...
class RecognizerNewButtonTest {
    ...
    fun setup() {
        // Mock dependencies
        speechKitAuth = mockk(relaxed = true) {
            coEvery { getAccessToken() } returns "fake-token"
        }

        microphoneSharedFlow = mockk(relaxed = true) {
    ...
    @Test
    fun `click should toggle state from Idle to Start`() = runTest {
    ...
    @Test
    fun `click should toggle state from Start to Idle`() = runTest {
